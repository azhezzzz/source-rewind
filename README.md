...
SourceRewind 是一个偶尔使用的一次性采集工具，不是长期运行的服务，也不是通用浏览器自动化框架。一次运行连接一个本地或远程 Chrome，按实际访问过程采集前端资源，生成报告后结束；采集期间可以手动操作浏览器，也可以在另一个终端运行恢复命令。

工具优先覆盖常见的本地启动和远程连接场景。遇到异常时会直接报错退出，后续再针对具体场景修复；它不承诺复杂网络环境、超大 CDP 消息或所有浏览器变体下的长期稳定运行。
